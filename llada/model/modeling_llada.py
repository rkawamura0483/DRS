...
def init_weights(
    ...
    if config.init_fn == InitFnType.normal:
        ...
        if config.init_cutoff_factor is not None:
            ...
            nn.init.trunc_normal_(module.weight, mean=0.0,
                                  std=std, a=-cutoff_value, b=cutoff_value)
        else:
...
class RMSLayerNorm(LayerNormBase):
    ...
    def __init__(
        ...
        super().__init__(config, size=size,
                         elementwise_affine=elementwise_affine, eps=config.rms_norm_eps)
...
class RotaryEmbedding(nn.Module):
    ...
    def get_rotary_embedding(self, seq_len: int, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(device.type, enabled=False):
            ...
            inv_freq = 1.0 / (self.rope_theta ** (torch.arange(0,
                              dim, 2, device=device, dtype=torch.float) / dim))
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, block_end_index: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(q.device.type, enabled=False):
            ...
            if block_end_index is None:
                ...
                q_ = self.apply_rotary_pos_emb(
                    pos_sin[:, :, block_end_index.item(
                    ) - query_len: block_end_index.item(), :],
                    pos_cos[:, :, block_end_index.item(
                    ) - query_len: block_end_index.item(), :],
                    q_,
                )
    ...
    def attention(
        ...
        if layer_past is not None:
            ...
            if replace_position is None:
                ...
                replace_indices = replace_position.nonzero(
                    as_tuple=True)[1]  # [selected_length]
...
class LLaDASequentialBlock(LLaDABlock):
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            q, k, v = self.att_proj(self.attn_norm(
                x)).split(self.fused_dims, dim=-1)
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def reset_parameters(self):
        ...
        self.ff_norm.reset_parameters()
        # NOTE: the standard deviation for these weights does not depend on the layer.
        init_weights(self.config, self.q_proj,
                     d=self.config.d_model, layer_id=None)
        init_weights(self.config, self.k_proj,
                     d=self.config.d_model, layer_id=None)
        init_weights(self.config, self.v_proj,
                     d=self.config.d_model, layer_id=None)
        init_weights(self.config, self.ff_proj,
                     d=self.config.d_model, layer_id=None)
        init_weights(self.config, self.up_proj,
                     d=self.config.d_model, layer_id=None)  # new add
...
class LLaDAModel(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        blocks = [LLaDABlock.build(i, config, self.__cache)
    ...
    def reset_parameters(self):
        ...
        if hasattr(self.transformer, "wpe"):
            init_weights(self.config, self.transformer.wpe,
        ...
        if hasattr(self.transformer, "ff_out"):
            init_weights(self.config, self.transformer.ff_out,
...
class LLaDAModelLM(PreTrainedModel):
    ...
    _no_split_modules = ["LLaDABlock",
                         "LLaDASequentialBlock", "LLaDALlamaBlock"]
